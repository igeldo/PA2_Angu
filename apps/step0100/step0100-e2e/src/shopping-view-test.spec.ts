...
test('Initial page has right URL', async ({ page }) => {
  // arrange
  await page.goto(Routes.BASE_FRONTEND_URL);
  // act

  // assert
  await expect(page).toHaveURL(Routes.SHOPPING_URL);
});

test('Not found component button redirects to shopping view', async ({ shoppingPage }) => {
  // arrange
  await shoppingPage.goToNotExistingURl();
  let notFoundButton = shoppingPage.getLocatorFrom(TestIds.NOT_FOUND_BTN);

  // act
  await notFoundButton.click();

  // assert
  await expect(shoppingPage.getPage()).toHaveURL(Routes.SHOPPING_URL);
});
...
test('Not found component is present when uknown url is called', async ({ shoppingPage }) => {
  ...
  // assert
  await expect(header).toHaveText(TestIds.NOT_FOUND_HEADER_TEXT);
